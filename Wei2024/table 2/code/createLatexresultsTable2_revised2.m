function createLatexresultsTable2_revised2()
...
for peak = 1:3    
    fid = fopen(sprintf('D:/Fabien/paperMiaomiaoLatest/allfigures/figures/common/resultsTable2P%d_revised2.txt', peak),'wt');
    ...
    for i=1:numel(names)
        ...
        for k=1:4
            ...
            if(idx == 8)
                fprintf(fid, '\\gradient{%.2f}', rmseLSTM(peak,k));
            end
            if(idx == 9)
                fprintf(fid, '\\gradient{%.2f}', rmseBayesianTracking(peak,k));
            end
            if(idx == 10)
                fprintf(fid, '\\gradient{%.2f}', rmseKalman(peak,k));
            end
            if(idx == 11)
                fprintf(fid, '\\gradient{%.2f}', rmseMOCAIPTracking(peak,k));
            end
...
% Avg
fid = fopen(sprintf('D:/Fabien/paperMiaomiaoLatest/allfigures/figures/common/resultsTable2all_revised2.txt'),'wt');
fprintf(fid, '\\resizebox{\\textwidth}{!}{%%\n');
fprintf(fid, '\\begin{tabular}{ c | l P{2.5cm} P{2.5cm} P{2.5cm} P{2.5cm} } \n');
fprintf(fid, '\\toprule \n');
...
for i=1:numel(names)
    ...
    for k=1:4
        ...
        if(idx==8)
            fprintf(fid, '\\gradient{%.2f}', (rmseLSTM(1,k)+rmseLSTM(2,k)+rmseLSTM(3,k))./3);            
        end
        if(idx==9)
            fprintf(fid, '\\gradient{%.2f}', (rmseBayesianTracking(1,k)+rmseBayesianTracking(2,k)+rmseBayesianTracking(3,k))./3);                    
        end
        if(idx==10)
            fprintf(fid, '\\gradient{%.2f}', (rmseKalman(1,k)+rmseKalman(2,k)+rmseKalman(3,k))./3);                      
        end
        if(idx==11)
            fprintf(fid, '\\gradient{%.2f}', (rmseMOCAIPTracking(1,k)+rmseMOCAIPTracking(2,k)+rmseMOCAIPTracking(3,k))./3);           
        end
