function validateforBasicAlgorithmFabforTable1()

nFold = 3;

%options.learnerList = { @trainRidgeRegression, @trainKernelRidgeRegression};
%options.testList =    { @testRidgeRegression,  @testKernelRidgeRegression};
...
% train peak detection models
for learner=1:numel(options.learnerList)
    for fold=1:nFold
        idxTrain = ismember(dataLearn.patient, uPatient(CV.training(fold)));
        for peak=1:3
            models{learner}.model{fold, peak} = feval(options.learnerList{learner}, dataLearn.X(idxTrain,:), dataLearn.Y(idxTrain, peak), optionsList{learner});
        end
    end
end
    ...
    for learner=1:numel(options.testList)
        for peak=1:3
            for fold=1:nFold                
                ...
                errI{fold} = (dataLearn.Y(idxTest,peak) - Yhat);
                sseI{fold} = (dataLearn.Y(idxTest,peak) - Yhat).^2;
                ssyI{fold} = (dataLearn.Y(idxTest,peak) - mean(dataLearn.Y(idxTest,peak))).^2;
                
                gtValue = (dataLearn.Y(idxTest,peak) .* norm.normFactY.maxValueY(peak)) + norm.normFactY.minValueY(peak);
                predValue = (Yhat .* norm.normFactY.maxValueY(peak)) + norm.normFactY.minValueY(peak);
                seeI_corrected{fold} = (gtValue - predValue) .^2;
                %Yhat3(:,peak) = Yhat;%wmm20200821
            end
            
            sse = sum(cell2mat(seeI_corrected'));            % SSE = sum( (Y - Yhat) ^2)
            ssy = sum(cell2mat(ssyI'));            % SSY = sum( (Y - mean(Y) ^2)    
            ...
            meanAbsErr{peak}(noiseIdx, learner) = mean(abs(cell2mat(errI'))) .* (norm.normFactY.maxValueY(peak)-norm.normFactY.minValueY(peak));  
            ...
            meanValue(peak) = mean((dataLearn.Y(:,peak) .* norm.normFactY.maxValueY(peak)) + norm.normFactY.minValueY(peak));
        end
    end
end
