...
for peak=1:3
    ...
    tracking0(:,peak) = LSTMTracking6(testLatencysN0(idxTest,peak), Gt(:,peak), idxTrain, idxTest, sequence(peak));
    tracking1(:,peak) = LSTMTracking6(testLatencysN1(idxTest,peak), Gt(:,peak), idxTrain, idxTest, sequence(peak));
    tracking2(:,peak) = LSTMTracking6(testLatencysN2(idxTest,peak), Gt(:,peak), idxTrain, idxTest, sequence(peak));
    tracking3(:,peak) = LSTMTracking6(testLatencysN3(idxTest,peak), Gt(:,peak), idxTrain, idxTest, sequence(peak));
end
...
for noiseLevel=0:3
    for peak=1:3
        ...
        rmseLSTM(peak, noiseLevel+1) = sqrt(sse ./ numel(sseI));
    end
end

save('D:\Fabien\paperMiaomiaoLatest\allfigures\figures\common\rmse_Latency_LSTM2.mat', "rmseLSTM");
