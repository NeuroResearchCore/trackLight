function [model, dataLearn] = performLearnICPRegModelMiao(options)
...
% load('E:\mcode\ICP\code - Copy\learnPeakDetector\ICPdatabase.mat', 'vv', 'l1', 'l2', 'l3', 'patient');
%load('./ArtificialICPdata0%noise400.mat', 'vv', 'l1', 'l2', 'l3', 'patient');%wmm20200506,ArtificialICPdata5%noise.mat
% load('E:\code - Copy\learnPeakDetector\ArtificialData-NoNnoise400.mat', 'vv', 'l1', 'l2', 'l3');
load('.\ArtificialDataNoNnoise400-1.mat', 'vv', 'l1', 'l2', 'l3');%same as simulatedData.mat
patient = 1:1:length(l1);
...
%     dataLearn.idxShift = idxShift;%wmm20200918
    l1(i) = l1(i) - idxShift;
    l2(i) = l2(i) - idxShift;
    l3(i) = l3(i) - idxShift;
end
...
%         dataLearn.RecoverCoefficient(i) = options.pulseLength ./ nnz(~isnan(X(i,:)));%wmm20200918
...
%  dataLearn.YUnormalized = [l1 l2 l3];                                       %wmm20200918
if(options.normalizeAUC)
    for i=1:numel(vv)
        X(i,:) = X(i,:) ./ sum(X(i,:));
        model.normFact.sum(i) = sum(X(i,:));
    end
end
...
if(options.removeOutliers)
    ...
    model.normFact.sum = model.normFact.sum(~outliers);
end
...
%    X = (X - repmat(normFact.median, size(X,1),1)) ./ repmat(normFact.std, size(X,1),1);    
   % X = NormalizeFea(X, 0);    
    ...
    model.normFact.median = median(X,1);
    model.normFact.std = nanstd(X,0,1);    
    X = (X - repmat(model.normFact.median, size(X,1),1)) ./ repmat(model.normFact.std, size(X,1),1);    
   % X = NormalizeFea(X, 0);    
    model.normFact.min = min(X(:));
    model.normFact.max = max(X(:));    
   
    X = (X - model.min(X(:))) ./ (model.max(X(:)) - model.min(X(:)));
end
