...
for peak=1:3
    ...
    Yhat = (filteredN(:,peak) - norm.normFactY.minValueY(peak)) ./ norm.normFactY.maxValueY(peak);
...
%    ssy = nansum(ssyI);            % SSY = sum( (Y - mean(Y) ^2)
    ...
    rmseMOCAIP{peak} = sqrt(sse ./ numel(sseI));
end

%save('./rmse_Latency_MOCAIP.mat', "rmseMOCAIP");
...
set(h,'Units','Inches');
 ...
 print(h, './MOCAIPLatency_revised3','-dpng','-r200');
 print(h, './MOCAIPLatency_revised3','-deps2c','-r200');
 close;
