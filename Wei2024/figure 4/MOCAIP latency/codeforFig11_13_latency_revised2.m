...
for noiseLevel=0:3
    for peak=1:3
        for cursor=1:winSize:size(testLatencysN,1)-winSize
            svalues = eval([sprintf('testLatencysN%d(cursor:cursor+winSize,peak);',noiseLevel)]);
            C = clusterdata(svalues, 3);
            [cnt_unique, unique_a] = hist(C, unique(C));
            [v, idx] = max(cnt_unique);

            eval([sprintf('filteredN%d(cursor:cursor+winSize-1,peak) =  repmat(mean(svalues(C==idx)), 1, winSize);',noiseLevel)]);
        end
    end
end
...
gt = testLatencysN(1:318,:);

for noiseLevel=0:3
    for peak=1:3
        gt_peak = gt(:,peak);
        gt_peak = (gt_peak - norm.normFactY.minValueY(peak)) ./ norm.normFactY.maxValueY(peak);
        op = sprintf('(filteredN%d(:,peak) - norm.normFactY.minValueY(peak)) ./ norm.normFactY.maxValueY(peak)',noiseLevel);
        Yhat = eval(op);
        sseI = (gt_peak - Yhat ).^2;
        sse = nansum(sseI);            % SSE = sum( (Y - Yhat) ^2)
        rmseMOCAIP(peak, noiseLevel+1) = sqrt(sse ./ numel(sseI));
    end
end

save('D:\Fabien\paperMiaomiaoLatest\allfigures\figures\common\rmse_Latency_MOCAIP2.mat', "rmseMOCAIP");
