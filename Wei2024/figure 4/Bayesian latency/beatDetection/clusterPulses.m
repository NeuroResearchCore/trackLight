 function [domPulses, pulsesTime, mICP] = clusterPulses(aCluster, timing, AlgoParam)
%
...
for j=1:nbclusters
  ...
  %  for i=1:numel(vv)
        idx = linspace(1, nnz(~isnan(tmp)), pulseLength);
        
        tmp = interp1(1:nnz(~isnan(tmp)), tmp(~isnan(tmp)), idx, 'pchip');
 %   end
%end            
...
for i=1:size(domPulses,1)
    domPulses(i,:) = domPulses(i,:) ./ sum(domPulses(i,:));
end
%domPulses = X;
...
%     %--
%     
%     domPulses(i,:) = interp1(x, domPulses2(i,:), xi, 'pchip'); % cubic
