function [LTInfo, AmpInfo, CurvInfo, SlopeInfo, AuxInfo] = computePulseMetrics_mm(v, ppk, fs, onsetFun)

mICP = mean(v);
sig_diff1 = diff1(v, 5);
sig_diff2 = diff1(sig_diff1, 5);

curv = [abs(sig_diff2(:))]./((1+sig_diff1(:).*sig_diff1(:)).^(1.5));%zeros(10,1); 
acurv = [(sig_diff2(:))]./((1+sig_diff1(:).*sig_diff1(:)).^(1.5));%zeros(10,1);
...
LTInfo.Lv1p1 = LTInfo.Lv1p1 +lt;
LTInfo.Lv1p2 = LTInfo.Lv1p2 +lt;
LTInfo.Lv1p3 = LTInfo.Lv1p3 +lt;
%wmm20200606
