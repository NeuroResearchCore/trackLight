...
%save('./noisyTrackingData.mat', 'testLatencysN');
...
for peak=1:3
    ...
    Yhat = (tracking1(:,peak) - norm.normFactY.minValueY(peak)) ./ norm.normFactY.maxValueY(peak);
...
%    ssy = nansum(ssyI);            % SSY = sum( (Y - mean(Y) ^2)
    ...
    rmseBayesianTracking{peak} = sqrt(sse ./ numel(sseI));
end
...
set(h,'Units','Inches');
 ...
 print(h, './BayesianLatency_revised3','-dpng','-r200');
 print(h, './BayesianLatency_revised3','-deps2c','-r200');
 close;
