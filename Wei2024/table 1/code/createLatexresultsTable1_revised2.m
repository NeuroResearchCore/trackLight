...
load('./step1Table1_latest_mocaip.mat','meanAbsErr', 'SD');
SDMocaip = SD;
avgMAEMOCAIP =  (meanAbsErr{1} + meanAbsErr{2} + meanAbsErr{3})/3;
meanAbsErrMOCAIP = meanAbsErr;

load('../step1Table1_latest.mat', 'meanAbsErr', 'SD');
...
for peak = 1:3    
    fid = fopen(sprintf('./resultsTable1P%d_revised3.txt', peak),'wt');
    ...
    for i=1:numel(names)
        ...
        for k=1:4
            if(idx~=7)
                fprintf(fid, '\\gradient{%.1f}', meanAbsErr{j}(k,idx));
            else
                fprintf(fid, '\\gradient{%.1f}', meanAbsErrMOCAIP{j}(k,1));            
            end
            if(k~=4)
                fprintf(fid, ' & ');
            end
...
% Avg
fid = fopen(sprintf('./resultsTable1all_revised3.txt'),'wt');
fprintf(fid, '\\resizebox{\\textwidth}{!}{%%\n');
fprintf(fid, '\\begin{tabular}{ l P{2.5cm} P{2.5cm} P{2.5cm} P{2.5cm} } \n');
fprintf(fid, '\\toprule \n');
...
%    j = peak;
    for k=1:4
        if(idx~=7)
            fprintf(fid, '\\gradient{%.1f}', avgMAE(k,idx));
        else
            fprintf(fid, '\\gradient{%.1f}', avgMAEMOCAIP(k,1));                        
        end
        if(k~=4)
            fprintf(fid, ' & ');
        end
