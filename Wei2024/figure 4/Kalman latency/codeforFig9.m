...
%save('./rmse_Latency_Kalman.mat', "rmseKalman");

%---------------------

% figure;
% plot(testLatencysN,'b');
% hold on;
% plot(tracking2(:,1:3),'r');
% plot(latencys,'g');
% legend('Latencys with 5% noise','','','Kalman prediction','','','Real peak latency','','');
% xlabel('Index');
% ylabel('Latency(ms)');
...
cl = parula(6);
t2 = annotation('textbox',pt2,'string','Kalman filter','edgecolor','none','fitboxtotext','on');
bh = 0.025;
bw = 0.008;
...
set(h,'Units','Inches');
 ...
 print(h, './KalmanLatency_revised3','-dpng','-r200');
 print(h, './KalmanLatency_revised3','-deps2c','-r200')
 close;
